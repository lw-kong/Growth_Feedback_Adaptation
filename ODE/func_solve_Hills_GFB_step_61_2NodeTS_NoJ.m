...
function [t_switch,x_all,x_after,equili] = func_solve_Hills_GFB_step_61_2NodeTS_NoJ(...
...
%% before signal
x0 = 0.1*ones(1,num_node);

[~,x] = ode4(@(t,x) eq_Hills_GFB_2NodeTS_NoNoise_NoJ(t,x,a_rest,k_node,k_link,k_growth),0:t_step:t_unit,x0);
x_all = x;
...
%% after signal
x0(:) = x_all(end,:);
[~,x] = ode4(@(t,x) eq_Hills_GFB_2NodeTS_NoNoise_NoJ(t,x,a_signal,k_node,k_link,k_growth),0:t_step:t_unit,x0);
x_after = x;
