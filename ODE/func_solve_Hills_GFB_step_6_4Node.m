...
function [t_switch,x_all,x_after,equili] = func_solve_Hills_GFB_step_6_4Node(...
...
[~,x] = ode4(@(t,x) eq_Hills_GFB_4Node_NoNoise(t,x,a_rest,k_node,k_linkfull,k_growth,J),0:t_step:t_unit,x0);
x_all = x;
...
while flag_eq ~= 2
    ...
    if ( std(x(:,1)/mean(x(:,1))) < std_relative_thres ...
        ...
        && std(x(:,3)/mean(x(:,3))) < std_relative_thres ...
        && std(x(:,3)/mean(x(:,4))) < std_relative_thres ) ...
        ||( std(x(:,1)) < std_abs_thres ...
        && std(x(:,2)) < std_abs_thres ...
        && std(x(:,3)) < std_abs_thres ...
        && std(x(:,4)) < std_abs_thres )
        flag_eq = flag_eq + 1;
    else
        flag_eq = 0;
    end
...
%% after signal
x0(:) = x_all(end,:);
[~,x] = ode4(@(t,x) eq_Hills_GFB_4Node_NoNoise(t,x,a_signal,k_node,k_linkfull,k_growth,J),0:t_step:t_unit,x0);
x_after = x;
