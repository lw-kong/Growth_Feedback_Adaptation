...
function [t_switch,x_all,x_after,equili] = func_solve_Hills_GFB_step_61_2NodeTS(...
    num_node,k_node,k_link,k_growth,J,...
...
% change from func_solve_Hills_GFB_step_6_2NodeTS
%    remove the relative std criterion for equilibrium
...
[~,x] = ode4(@(t,x) eq_Hills_GFB_2NodeTS_NoNoise(t,x,a_rest,k_node,k_link,k_growth,J),0:t_step:t_unit,x0);
x_all = x;
...
%% after signal
x0(:) = x_all(end,:);
[~,x] = ode4(@(t,x) eq_Hills_GFB_2NodeTS_NoNoise(t,x,a_signal,k_node,k_link,k_growth,J),0:t_step:t_unit,x0);
x_after = x;
